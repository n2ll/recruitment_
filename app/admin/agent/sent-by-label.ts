 ...
 *
 * 매핑 규칙:
 *   - system-* (system-auto, system-bulk, system-onboarding-reminder 등) → "자동 발송 메시지"
 *   - danggeun-manual                                                   → "매니저"
 *   - agent / agent-practice                                            → "AI 에이전트"
 *   - 그 외 danggeun-* (start / practice-start / recommend)              → "AI 에이전트"
 *   - 그 외(관리자/dispatch 등)는 원본 그대로 표시
 */
export function sentByLabel(raw: string | null | undefined): string {
  if (!raw) return "";
  if (raw === "system-auto" || raw.startsWith("system-")) return "자동 발송 메시지";
